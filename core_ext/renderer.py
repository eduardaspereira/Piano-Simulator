...
class Renderer:
    def __init__(self, clear_color=[0, 0, 0]):
        ...
        self._screen_mesh_list = []
        self.screen_width = 1600  # Default values, will be updated
    ...
    def add_screen_mesh(self, mesh):
        """Add a mesh to be rendered in screen space"""
        self._screen_mesh_list.append(mesh)
        # Locate uniforms when mesh is added
        self._locate_uniforms(mesh)

    def _locate_uniforms(self, mesh):
        """Locate all uniforms in the material"""
        if not hasattr(mesh, 'material') or not hasattr(mesh.material, 'uniform_dict'):
            return
            
        for uniform_name, uniform_object in mesh.material.uniform_dict.items():
            uniform_object.locate_variable(mesh.material.program_ref, uniform_name)
    ...
    def _render_scene(self, scene, camera):
        ...
        for mesh in mesh_list:
            if not mesh.visible:
                continue
            self._render_mesh(mesh, camera)

    def _render_mesh(self, mesh, camera=None):
        GL.glUseProgram(mesh.material.program_ref)
        GL.glBindVertexArray(mesh.vao_ref)
        
        # Only update these matrices for 3D objects
        if camera is not None:
            if "modelMatrix" in mesh.material.uniform_dict:
                mesh.material.uniform_dict["modelMatrix"].data = mesh.global_matrix
            if "viewMatrix" in mesh.material.uniform_dict:
                mesh.material.uniform_dict["viewMatrix"].data = camera.view_matrix
            if "projectionMatrix" in mesh.material.uniform_dict:
                mesh.material.uniform_dict["projectionMatrix"].data = camera.projection_matrix
    ...
    def _render_screen_elements(self):
        GL.glDisable(GL.GL_DEPTH_TEST)
        
        for mesh in self._screen_mesh_list:
            if not mesh.visible:
                continue
